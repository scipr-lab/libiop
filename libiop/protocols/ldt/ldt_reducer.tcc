...
public:
        ...
        const std::vector<std::vector<FieldT> > &constituent_oracle_evaluations) const
    {
        ...
        std::vector<FieldT> result(constituent_oracle_evaluations[0].size(), FieldT::zero());
        ...
        for (std::size_t i = 0; i < constituent_oracle_evaluations.size(); ++i)
        {
            ...
            {
                result[j] += this->coefficients_[i] * constituent_oracle_evaluations[i][j];
            }
        }
        ...
        for (std::size_t i = 0; i < this->submaximal_oracle_indices_.size(); ++i)
        {
            ...
            const std::vector<FieldT> bump_factors =
                subset_element_powers(this->codeword_domain_,
                                      this->max_degree_ - this->input_oracle_degrees_[submaximal_oracle_index]);

            for (std::size_t j = 0; j < result.size(); ++j)
            {
                result[j] += this->coefficients_[this->num_input_oracles_ + i] *
                    bump_factors[j] *
                    constituent_oracle_evaluations[submaximal_oracle_index][j];
            }
        }
