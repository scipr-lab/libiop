    ...
    domain_type_(domain_type)
{
    ...
    this->multi_lincheck_params_ = multi_lincheck_parameters<FieldT>(
...
template<typename FieldT>
size_t encoded_aurora_parameters<FieldT>::locality() const
{
    /** fw, f_Az, f_Bz, f_Cz*/
    const size_t arithmetization_locality = 4;
    /** rowcheck outputs a virtual oracle */
    const size_t rowcheck_locality = 0;
    const size_t lincheck_locality = this->multi_lincheck_params_.locality();
    return arithmetization_locality + rowcheck_locality + lincheck_locality;
}
...
public:
        ...
        const std::vector<std::vector<FieldT> > &constituent_oracle_evaluations) const
    {
        ...
        const std::vector<FieldT> &fw = constituent_oracle_evaluations[0];
        ...
        {
            result.emplace_back(fw[i] * input_vp_over_codeword_domain[i] + f_1v_over_codeword_domain[i]);
        }
    ...
    params_(params)
{
    ...
    this->input_variable_domain_ = this->variable_domain_.get_subset_of_order(this->constraint_system_.num_inputs() + 1);
...
void encoded_aurora_protocol<FieldT>::register_witness_oracles()
{
    ...
    // TODO: this takes a copy internally; in future consider
    // propagating encoded_aurora_protocol interface to take a shared pointer to
    // a constraint system and have R1CS example generator return
    // shared pointer to a constraint system.
    //
    // but for now this is not too heavyweight, so we leave it to be
    // revisited later.
    std::shared_ptr<r1cs_constraint_system<FieldT> > cs =
        std::make_shared<r1cs_constraint_system<FieldT> >(this->constraint_system_);

    this->r1cs_A_ = std::make_shared<r1cs_sparse_matrix<FieldT> >(cs, r1cs_sparse_matrix_A);
    this->r1cs_B_ = std::make_shared<r1cs_sparse_matrix<FieldT> >(cs, r1cs_sparse_matrix_B);
    this->r1cs_C_ = std::make_shared<r1cs_sparse_matrix<FieldT> >(cs, r1cs_sparse_matrix_C);
...
{
    enter_block("register challenge");
    this->multi_lincheck_->register_challenge();
    leave_block("register challenge");
}
...
template<typename FieldT>
void encoded_aurora_protocol<FieldT>::calculate_ABCz(const std::vector<FieldT> &variable_assignment){
    /** This assumes variable assignment z is structured as (1, v, w).
     *  This does not require changes in the multiplicative setting, as we
     *  are building evaluations by taking the dot product of a given row and z,
     *  so a coefficient in the ith column of the matrix correctly corresponds to
     *  the ith element of z. */
    const r1cs_constraint_system<FieldT> &cs = this->constraint_system_;
...
void encoded_aurora_protocol<FieldT>::compute_fprime_ABCz_over_codeword_domain(){
    ...
    polynomial<FieldT> f_Az(std::move(
            IFFT_over_field_subset<FieldT>(this->Az_, this->constraint_domain_)));
    polynomial<FieldT> f_Bz(std::move(
            IFFT_over_field_subset<FieldT>(this->Bz_, this->constraint_domain_)));
    polynomial<FieldT> f_Cz(std::move(
            IFFT_over_field_subset<FieldT>(this->Cz_, this->constraint_domain_)));
    ...
    const r1cs_auxiliary_input<FieldT> &auxiliary_input)
{
    ...
    this->multi_lincheck_->submit_witness_oracles();
    ...
    */
    ...
    this->calculate_ABCz(variable_assignment);

    leave_block("Compute A/B/Cz");

    enter_block("Compute f_{A/B/Cz} over codeword domain");
    this->compute_fprime_ABCz_over_codeword_domain();
    ...
    this->IOP_.submit_oracle(this->fw_handle_, this->fw_over_codeword_domain_);
    this->IOP_.submit_oracle(this->fAz_handle_, this->fprime_Az_over_codeword_domain_);
    this->IOP_.submit_oracle(this->fBz_handle_, this->fprime_Bz_over_codeword_domain_);
    this->IOP_.submit_oracle(this->fCz_handle_, this->fprime_Cz_over_codeword_domain_);
...
{
    std::vector<oracle_handle_ptr> result = this->multi_lincheck_->get_all_oracle_handles();
