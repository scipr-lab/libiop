...
multi_lincheck_parameters<FieldT>::multi_lincheck_parameters(
...
template<typename FieldT>
bool multi_lincheck_parameters<FieldT>::make_zk() const
{
    return this->make_zk_;
}

template<typename FieldT>
field_subset_type multi_lincheck_parameters<FieldT>::domain_type() const
{
    return this->domain_type_;
}

template<typename FieldT>
size_t multi_lincheck_parameters<FieldT>::multi_lincheck_repetitions() const
{
    return this->multi_lincheck_repetitions_;
}

template<typename FieldT>
void multi_lincheck_parameters<FieldT>::override_security_parameter(const size_t multi_lincheck_repetitions)
{
...
template<typename FieldT>
size_t multi_lincheck_parameters<FieldT>::locality() const
{
...
template<typename FieldT>
long double multi_lincheck_parameters<FieldT>::achieved_interactive_soundness() const
{
    const long double field_size_bits = (long double)(soundness_log_of_field_size_helper<FieldT>(FieldT::zero()));
...
template<typename FieldT>
void multi_lincheck_parameters<FieldT>::print() const
{
...
multi_lincheck<FieldT>::multi_lincheck(
    ...
    const multi_lincheck_parameters<FieldT> params):
    IOP_(IOP),
    ...
    params_(params)
{
    ...
    const bool cache_evaluations = true;
...
template<typename FieldT>
void multi_lincheck<FieldT>::submit_witness_oracles()
{
    for (size_t i = 0; i < this->params_.multi_lincheck_repetitions(); i++)
    {
        if (this->params_.make_zk()) {
...
{
    enter_block("Calculate and submit proof");
    for (size_t i = 0; i < this->params_.multi_lincheck_repetitions(); i++)
    {
    ...
    leave_block("Calculate and submit proof");
}
