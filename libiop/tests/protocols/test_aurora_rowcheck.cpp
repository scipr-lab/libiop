              ...
              bool exp_pass)
{
    ...
    const std::vector<std::vector<FieldT> > codewords(
        {Az_over_codeword_domain, Bz_over_codeword_domain, Cz_over_codeword_domain});
    const std::vector<FieldT> sumcheck_evals = rowcheck_oracle.evaluated_contents(codewords);
    // ensure it sums to 0
    const polynomial<FieldT> rowcheck_poly(
        IFFT_over_field_subset<FieldT>(sumcheck_evals, codeword_domain));
    ...
    for (std::size_t i = 0; i < 10; i++) {
        ...
        EXPECT_TRUE(eval == sumcheck_evals[evaluation_index]) << "rowcheck evaluation at point was inconsistent";
    }
}
