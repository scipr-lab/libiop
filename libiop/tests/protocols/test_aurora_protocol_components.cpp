    ...
    const field_subset<FieldT> codeword_domain, const field_subset_type domain_type)
{
    ...
    encoded_aurora_protocol<FieldT> proto(IOP,
                                          ...
                                          r1cs_system.constraint_system_,
                                          params);
...
for (std::size_t variable_domain_dim = 7; variable_domain_dim < 9; variable_domain_dim++) {
    ...
    for (std::size_t make_zk_param = 0; make_zk_param < 2; make_zk_param++) {
        ...
        params.Az_vec = IFFT_over_field_subset(proto.fprime_Az_over_codeword_domain_, params.codeword_domain_);
        params.Bz_vec = IFFT_over_field_subset(proto.fprime_Bz_over_codeword_domain_, params.codeword_domain_);
        params.Cz_vec = IFFT_over_field_subset(proto.fprime_Cz_over_codeword_domain_, params.codeword_domain_);
        std::vector<FieldT> fz_over_codeword_domain = proto.fz_oracle_->evaluated_contents({proto.fw_over_codeword_domain_});
        ...
        test_fw(proto.fw_over_codeword_domain_, proto.f_1v_coefficients_,
            &params);
...
for (std::size_t variable_domain_dim = 6; variable_domain_dim < 8; variable_domain_dim++) {
    ...
    for (std::size_t make_zk_param = 0; make_zk_param < 2; make_zk_param++) {
        ...
        /* TODO: add methods for generating elements outside certain subspaces */
