    ...
    bool exp_pass)
{
    ...
    multi_lincheck_parameters<FieldT> params(dummy_security_parameter,
    ...
    multi_lincheck.submit_witness_oracles();
    ...
    const std::vector<FieldT> r_Mz)
{
    ...
    std::vector<std::vector<FieldT>> constituent_codewords;
    constituent_codewords.emplace_back(fz_over_codeword_domain);
    for (std::size_t i = 0; i < Mzs_over_codeword_domain.size(); i++) {
        constituent_codewords.emplace_back(Mzs_over_codeword_domain[i]);
    }

    std::vector<FieldT> lincheck_q = multi_lincheck.evaluated_contents(constituent_codewords);
