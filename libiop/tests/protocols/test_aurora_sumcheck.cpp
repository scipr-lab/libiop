...
{
    // TODO: Add convenience method to generate shift outside of a given subspace
    FieldT shift;
    ...
    const std::vector<FieldT> masking_poly_evals = IOP.get_oracle_evaluations(
        std::make_shared<oracle_handle>(sumcheck.get_masking_poly_oracle_handle()));
    const std::vector<FieldT> h_evals = IOP.get_oracle_evaluations(
        std::make_shared<oracle_handle>(sumcheck.get_h_oracle_handle()));
