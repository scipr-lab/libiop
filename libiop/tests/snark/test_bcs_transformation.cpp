...
bcs16_transformation_parameters<FieldT> get_bcs_parameters()
{
...
void run_test(const field_subset<FieldT> codeword_domain,
    ...
    bcs16_transformation_parameters<FieldT> bcs_parameters = get_bcs_parameters<FieldT>();
    /* Decide on query positions according to round_params */
    
    /* Run prover */
    bcs16_prover<FieldT> prover_IOP(bcs_parameters);
    domain_handle codeword_domain_handle = prover_IOP.register_domain(codeword_domain);
    dummy_protocol<FieldT> proto(
        prover_IOP, num_oracles_per_round, num_rounds, round_params, codeword_domain_handle, make_zk);
    ...
    bcs16_transformation_transcript<FieldT> transcript = prover_IOP.get_transcript();

    /* Run verifier */
    bcs16_verifier<FieldT> verifier_IOP(bcs_parameters, transcript);
    codeword_domain_handle = verifier_IOP.register_domain(codeword_domain);
    dummy_protocol<FieldT> verifyier_dummy_proto(
        verifier_IOP, num_oracles_per_round, num_rounds, round_params, codeword_domain_handle, make_zk);
...
TEST(BasicOneRoundTests, BCSTest) {
    ...
    run_single_round_test<FieldT>(codeword_domain, num_oracles, round_params, query_positions, zk, expected_proof_size);
